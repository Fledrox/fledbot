...
module.exports = (client, message) => {
	// Ignores if it doesn't have the command prefix, if the author is a bot or if bot does not have permissions to send message.
	...
	// Tells if it's not in #bot-testing or #bots
	...
	// Ignores if command doesn't exist
	const command = client.commands.get(commandName) || client.commands.find((cmd) => cmd.aliases && cmd.aliases.includes(commandName));
	if (!command) return;

	// If command is done in DMs and "guildOnly" is set to TRUE, ignores command
